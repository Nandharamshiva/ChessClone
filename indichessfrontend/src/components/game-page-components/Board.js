...
const Board = ({addMove}) => {
  ...
  const gameType = location.state?.gameType || "1v1";
  
  // Generate or retrieve gameId from localStorage (shared across tabs for same game mode)
  // This ensures both tabs join the same game room
  const gameIdKey = `gameId_${gameType}`;
  ...
  const playerUsername = sessionStorage.getItem("username") || "Player1"; // Get username from sessionStorage
  ...
  const applyOpponentMove = (move) => {
    try {
      ...
      // Validate algebraic notation format (e.g., "c2")
      if (typeof from !== 'string' || typeof to !== 'string' || from.length !== 2 || to.length !== 2) {
        console.error("Invalid move format. Expected algebraic notation (e.g., 'c2'):", { from, to });
        return;
      }

      // Parse algebraic notation: 'c2' -> col=2, row=6
      const fromCol = from.charCodeAt(0) - 'a'.charCodeAt(0);  // 'a'-'h' -> 0-7
      const fromRow = 8 - parseInt(from[1]);                    // '1'-'8' -> 7-0
      ...
      if (fromRow < 0 || fromRow > 7 || fromCol < 0 || fromCol > 7 || 
        ...
        console.error("Move coordinates out of bounds:", { fromRow, fromCol, toRow, toCol });
        return;
      }

      const piece = board[fromRow][fromCol];
      if (!piece) {
        console.warn("No piece at source position:", { from, fromRow, fromCol });
        return;
      }
      ...
      if (promotionPiece && (piece === "p" || piece === "P")) {
      ...
      if ((piece === "K" || piece === "k") && Math.abs(toCol - fromCol) === 2) {
        if (toCol > fromCol) {
          // Short castle
          newBoard[fromRow][7] = "";
          newBoard[fromRow][5] = newBoard[fromRow][7] === "" ? (piece === "K" ? "R" : "r") : "";
        } else {
          // Long castle
          newBoard[fromRow][0] = "";
          newBoard[fromRow][3] = newBoard[fromRow][0] === "" ? (piece === "K" ? "R" : "r") : "";
        }
        castled = true;
      }
      ...
      console.log("Opponent move applied successfully:", { from, to, piece });
    } catch (error) {
      console.error("Error applying opponent move:", error, move);
    }
  };
  ...
  useEffect(() => {
    ...
    if (gameType !== "bot") {
      GameWebSocketService.connect(gameId, playerUsername, () => {
        console.log("Connected to game:", gameId);
      });

      // Listen for opponent moves
      GameWebSocketService.onMoveReceived((message) => {
        console.log("Opponent move received:", message);
        console.log("Message type:", message.type);
        console.log("Message playerUsername:", message.playerUsername);
        console.log("Current playerUsername:", playerUsername);
        console.log("Message.move:", message.move);
        if (message.move && message.playerUsername !== playerUsername) {
          console.log("Applying opponent move:", message.move);
          applyOpponentMove(message.move);
        } else {
          console.log("Move is from self or no move data, skipping");
          console.log("Reason - move exists:", !!message.move, "Different player:", message.playerUsername !== playerUsername);
        }
      });

      // Listen for game messages (resign, draw offer, etc.)
      GameWebSocketService.onGameMessageReceived((message) => {
        console.log("Game message received:", message);
        if (message.type === "resign" && message.playerUsername !== playerUsername) {
          console.log(`${message.playerUsername} resigned!`);
        }
      });
    }
    ...
    return () => {
      ...
      if (gameType !== "bot") {
        GameWebSocketService.disconnect(playerUsername);
      }
    };
  }, []);
...
const isKingInCheck = () => {
  ...
  const updatePrevMove = (fr, fc, tr, tc, piece, capturedPiece, castled, fenBefore) => {
    ...
    // Send move via WebSocket for multiplayer games
    if (gameType !== "bot") {
      const from = String.fromCharCode('a'.charCodeAt(0) + fc) + sqnumfrom;
      const to = String.fromCharCode('a'.charCodeAt(0) + tc) + sqnumto;
      console.log("Attempting to send move:", { from, to, piece: piece.toLowerCase(), playerUsername, connected: GameWebSocketService.isWebSocketConnected() });
      if (GameWebSocketService.isWebSocketConnected()) {
        GameWebSocketService.sendMove(from, to, piece.toLowerCase(), null, playerUsername);
        console.log("Move sent via WebSocket");
      } else {
        console.warn("WebSocket not connected, move not sent");
      }
    }
